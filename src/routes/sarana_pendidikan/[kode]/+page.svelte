<script>
  ...
  import Body from "../../../components/sarana_pendidikan/Body.svelte";
  ...
  const loadWilayah = async () => {
    ...
    await axios
      .get(`${$urlApi}dashboard/${data.kode}/sarana_pendidikan`)
      .then(({ data }) => {
        let tempData = data.datas;
        infrastrukturPendidikan.set(
          tempData.filter(
            (item) =>
              item.kategori_variabel == "jumlah_infrastruktur_pendidikan"
          )
        );
      })
      .catch(({ response }) => {
