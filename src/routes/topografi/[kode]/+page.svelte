<script>
	...
	import Body from "../../../components/topografi/Body.svelte";
	...
	const loadWilayah = async () => {
		...
		await axios
			.get(`${$urlApi}dashboard/${data.kode}/topografi`)
			.then(({data})=>{
				let tempData = data.datas;
				topografiData.set({
					luas_wilayah: tempData.filter(item => item.kategori_variabel=='luas_wilayah'), 
					topografi: tempData.filter(item => item.kategori_variabel=='topografi'), 
					koordinat_wilayah: tempData.filter(item => item.kategori_variabel=='koordinat_wilayah'), 
					ketinggian_wilayah: tempData.filter(item => item.kategori_variabel=='ketinggian_wilayah'), 
					berbatasan_laut: tempData.filter(item => item.kategori_variabel=='berbatasan_laut'), 
					kawasan_hutan: tempData.filter(item => item.kategori_variabel=='kawasan_hutan'), 
					pemanfaatan_laut: tempData.filter(item => item.kategori_variabel=='pemanfaatan_laut'), 
					fungsi_hutan: tempData.filter(item => item.kategori_variabel=='fungsi_hutan')
				});
			}).catch(({ response })=>{
				console.error(response)
			})
