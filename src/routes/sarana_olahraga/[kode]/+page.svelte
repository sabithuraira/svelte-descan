<script>
  ...
  import Body from "../../../components/sarana_olahraga/Body.svelte";
  ...
  const loadWilayah = async () => {
    ...
    await axios
      .get(`${$urlApi}dashboard/${data.kode}/sarana_olahraga`)
      .then(({ data }) => {
        let tempData = data.datas;
        infrastrukturOlahraga.set(
          tempData.filter(
            (item) => item.kategori_variabel == "lapangan_olahraga"
          )
        );
      })
      .catch(({ response }) => {
