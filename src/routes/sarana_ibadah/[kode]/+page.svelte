<script>
  ...
  import Body from "../../../components/sarana_ibadah/Body.svelte";
  ...
  const loadWilayah = async () => {
    ...
    await axios
      .get(`${$urlApi}dashboard/${data.kode}/sarana_ibadah`)
      .then(({ data }) => {
        let tempData = data.datas;
        infrastrukturIbadah.set(
          tempData.filter(
            (item) => item.kategori_variabel == "jumlah_infrastruktur_ibadah"
          )
        );
      })
      .catch(({ response }) => {
