...
const infrastrukturEkonomi = writable([]);
const lembagaKeuangan = writable([]);
...
const Body = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  infrastrukturEkonomi.subscribe((value) => {
    if (value) {
      ...
      sum_infrastruktur_ekonomi = value.reduce(
        (acc, item) => {
  ...
  lembagaKeuangan.subscribe((value) => {
    if (value) {
      ...
      sum_lembaga_keuangan = value.reduce(
        (acc, item) => {
          return acc + Number(item.nilai);
        },
        0
  ...
  rekapContent = [
    {
      ...
      deskripsi: "Mencakup infrastruktur seperti Toko, Pasar, Minimarket, dan lain sebagainya.",
      data: infrastruktur_ekonomi
    },
    {
      total: sum_lembaga_keuangan,
      label: "Lembaga Keuangan",
      deskripsi: "Mencakup lembaga seperti Koperasi, Bank, dan lain sebagainya.",
